defmodule Atrium.SuperAdminsTest do
  ...
  describe "create_super_admin/1" do
    test "creates a super admin with a hashed password" do
      {:ok, sa} = SuperAdmins.create_super_admin(%{email: "sa_unit@atrium.example", name: "Ops", password: "correct-horse-battery-staple"})
    ...
    test "rejects duplicate email case-insensitively" do
      {:ok, _} = SuperAdmins.create_super_admin(%{email: "Sa_Unit@atrium.example", name: "Ops", password: "correct-horse-battery-staple"})
      {:error, cs} = SuperAdmins.create_super_admin(%{email: "sa_unit@atrium.example", name: "Ops2", password: "correct-horse-battery-staple"})
  ...
  describe "authenticate/2" do
    ...
    test "returns {:ok, sa} on correct password", %{sa: sa} do
      assert {:ok, found} = SuperAdmins.authenticate("sa_unit@atrium.example", "correct-horse-battery-staple")
      assert found.id == sa.id
    end

    test "returns {:error, :invalid_credentials} on wrong password" do
      assert {:error, :invalid_credentials} = SuperAdmins.authenticate("sa_unit@atrium.example", "wrong")
    end
    ...
    test "returns {:error, :suspended} when status is suspended", %{sa: sa} do
      ...
      assert {:error, :suspended} = SuperAdmins.authenticate("sa_unit@atrium.example", "correct-horse-battery-staple")
    end
  end
end
